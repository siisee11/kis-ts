...
export type KisDomesticContext = {
  http: AxiosInstance;
...
export type DomesticRequestOptions = {
  path: string;
  trId: string;
  trCont?: string;
...
export const domesticGet = async <T>(
  ctx: KisDomesticContext,
  options: DomesticRequestOptions,
  ...
  try {
    ...
    throw toKisError(
      error,
      options.errorMessage ?? "Failed to fetch domestic data.",
    );
  }
};

export const domesticPost = async <T>(
  ctx: KisDomesticContext,
  options: DomesticRequestOptions,
): Promise<AxiosResponse<T>> => {
  try {
    ...
    throw toKisError(
      error,
      options.errorMessage ?? "Failed to post domestic data.",
    );
  }
};
...
export const extractCursor = (
  ...
  trCont:
    ...
    readField(body, "TR_CONT"),
  fk: readField(body, keys.fk ?? "ctx_area_fk"),
  nk: readField(body, keys.nk ?? "ctx_area_nk"),
});
