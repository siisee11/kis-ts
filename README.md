...
```typescript
import { createKisClient, checkHoliday } from "@jasset/kis";
...
async function main() {
  ...
  // 2. Create Context
  const context = client.getDomesticContext(token.accessToken, {
    custType: "P", // Individual (P) or Business (B)
  });

  // 3. Call API
  const result = await checkHoliday(context, {
    bassDt: "20231225", // YYYYMMDD
  });
